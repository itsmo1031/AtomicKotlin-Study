...
private fun <T : KCallable<*>> Collection<T>.assertSingle(
    memberName: String,
    where: () -> String,
    what: () -> String
): T {
...
private fun checkParameter(index: Int, name: String, type: String, param: KParameter, funcName: String) {
    ...
    if (name.isNotEmpty()) {
        Assert.assertEquals(
            "Expected the $ordinal parameter named '$name' for ${funcName.decapitalize()}",
            name, param.name
        )
        Assert.assertEquals(
            "Expected the parameter '$name' of type '$type' for ${funcName.decapitalize()}",
    ...
    // this case isn't tested explicitly to avoid mysterious messages for students
        ...
        Assert.assertEquals(
            "Expected the $ordinal parameter of type '$type' for ${funcName.decapitalize()}",
            type, param.type.toString()
        )
    }
}
...
private fun checkFunctionOrPropertyIsExtension(
    ...
    Assert.assertTrue(
        "The '$name' $callable must be an *extension* $callable",
        functionOrGetter.parameters.size == 1
    )
    Assert.assertTrue(
        "Wrong receiver for '$name': should be '${expectedReceiver.simpleName}'",
