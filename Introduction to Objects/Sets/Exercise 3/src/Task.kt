...
fun countPercent(item: Set<String>, cart: Set<String>): Double {
    return (item.intersect(cart)).size.toDouble() / cart.size * 100
}
...
fun percentOther(groceryCart: Set<String>): Double =
    countPercent(groceryCart - meats - fruits - vegetables, groceryCart)
